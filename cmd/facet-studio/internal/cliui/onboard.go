...
func printOnboardPlain(logo string, encrypt bool, configPath string) {
	fmt.Printf("\n%s facet-studio is ready!\n", logo)
	fmt.Println("\nNext steps:")
	if encrypt {
		fmt.Println("  1. Set your encryption passphrase before starting facet-studio:")
		fmt.Println("       export FACET_STUDIO_KEY_PASSPHRASE=<your-passphrase>   # Linux/macOS")
		fmt.Println("       set FACET_STUDIO_KEY_PASSPHRASE=<your-passphrase>      # Windows cmd")
		fmt.Println("")
	...
	fmt.Println("")
	if encrypt {
		fmt.Println("  3. Chat: facet-studio agent -m \"Hello!\"")
	} else {
		fmt.Println("  2. Chat: facet-studio agent -m \"Hello!\"")
	}
}
...
func printOnboardFancy(logo string, encrypt bool, configPath string) {
	...
	ready := titleBarStyle().Render(logo+" facet-studio is ready!") + "\n"
	fmt.Println()
...
func buildOnboardingSteps(encrypt bool, configPath string) string {
	...
	if encrypt {
		b.WriteString("1. Set your encryption passphrase before starting facet-studio:\n")
		b.WriteString("   export FACET_STUDIO_KEY_PASSPHRASE=<your-passphrase>   # Linux/macOS\n")
		b.WriteString("   set FACET_STUDIO_KEY_PASSPHRASE=<your-passphrase>      # Windows cmd\n\n")
