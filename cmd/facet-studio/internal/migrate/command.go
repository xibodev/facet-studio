...
import (
	...
	"github.com/xibodev/facet-studio/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to facet-studio",
		Args:  cobra.NoArgs,
		Example: `  facet-studio migrate
  facet-studio migrate --from openclaw
  facet-studio migrate --dry-run
  facet-studio migrate --refresh
  facet-studio migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.facet-studio)")
