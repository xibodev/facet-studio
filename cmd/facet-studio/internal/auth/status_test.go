...
import (
	...
	pkgauth "github.com/xibodev/facet-studio/pkg/auth"
...
func setAuthStatusTestHome(t *testing.T) string {
	...
	tmpDir := t.TempDir()
	t.Setenv(config.EnvHome, filepath.Join(tmpDir, ".facet-studio"))
	return tmpDir
}
...
func TestAuthStatusCmdShowsCanonicalGoogleAntigravityAfterLegacyRefresh(t *testing.T) {
	...
	authPath := filepath.Join(tmpDir, ".facet-studio", "auth.json")
