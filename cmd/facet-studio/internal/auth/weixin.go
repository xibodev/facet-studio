...
import (
	...
	"github.com/xibodev/facet-studio/pkg/channels/weixin"
...
to authorize your account. On success, the bot token is saved to the facet-studio
config so you can start the gateway immediately.

Example:
  facet-studio auth weixin`,
...
func runWeixinOnboard(baseURL, proxy string, timeout time.Duration) error {
	...
	fmt.Println()
	fmt.Println("  facet-studio gateway")
	fmt.Println()
...
func printManualWeixinConfig(token, baseURL string) {
	fmt.Println()
	fmt.Println("Add the following to the channels section of your facet-studio config:")
	fmt.Println()
	fmt.Println(`  "weixin": {`)
