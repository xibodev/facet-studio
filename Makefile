.PHONY: all build kernel shell product install uninstall clean help test integration-test build-all lint-docs

# Build variables
BINARY_NAME=facet-studio
# THE TWO SHIPPABLE PRODUCTS. facet-studio is the FULL thing (the shell the user
# launches); facet-studio-kernel is that minus the web shell -- a complete
# agentic harness that runs on its own.
SHELL_NAME=facet-studio
KERNEL_NAME=facet-studio-kernel
BUILD_DIR=build
...
CONFIG_PKG=github.com/xibodev/facet-studio/pkg/config
...
PICOCLAW_HOME?=$(HOME)/.facet-studio
...
## kernel: Build facet-studio-kernel -- the harness, with no web shell
##
...
##
## It is the SAME source as the harness inside the full product; only the name
## differs, so a standalone install and a bundled one cannot drift.
kernel: generate
...
shell:
	...
	@cd web/frontend && npm run build:backend
...
ifeq ($(OS),Windows_NT)
	...
	@mkdir -p $(BUILD_DIR)
	@cd web/backend && GOOS=$(PLATFORM) GOARCH=$(ARCH) $(GO) build $(GOFLAGS) -ldflags "$(LDFLAGS)" -o ../../$(BUILD_DIR)/$(SHELL_NAME)$(EXT) .
endif
...
product: kernel shell
	@echo ""
	@echo "Full product in $(BUILD_DIR)/:"
	@echo "  $(SHELL_NAME)$(EXT)   <- the user launches this"
	@echo "  $(KERNEL_NAME)$(EXT)  <- it supervises this"

## build: Build the facet-studio binary for current platform
build: generate
...
## build-launcher: Build the facet-studio-launcher (web console) binary
build-launcher:
	@echo "Building facet-studio-launcher for $(PLATFORM)/$(ARCH)..."
ifeq ($(OS),Windows_NT)
	@$(POWERSHELL) "New-Item -ItemType Directory -Force -Path '$(BUILD_DIR)' | Out-Null"
	@$(MAKE) -C web build PLATFORM="$(PLATFORM)" ARCH="$(ARCH)" EXT="$(EXT)" OUTPUT="$(CURDIR)/$(BUILD_DIR)/facet-studio-launcher-$(PLATFORM)-$(ARCH)$(EXT)" GO_BUILD_TAGS="$(GO_BUILD_TAGS)"
	@$(POWERSHELL) "Copy-Item -LiteralPath '$(BUILD_DIR)/facet-studio-launcher-$(PLATFORM)-$(ARCH)$(EXT)' -Destination '$(BUILD_DIR)/facet-studio-launcher$(EXT)' -Force"
else
	@mkdir -p $(BUILD_DIR)
	@GOOS=$(PLATFORM) GOARCH=$(ARCH) $(MAKE) -C web build \
		OUTPUT="$(CURDIR)/$(BUILD_DIR)/facet-studio-launcher-$(PLATFORM)-$(ARCH)$(EXT)" \
		WEB_GO='$(WEB_GO)' \
		GO_BUILD_TAGS='$(GO_BUILD_TAGS)' \
		LDFLAGS='$(LDFLAGS)'
	@$(LNCMD) facet-studio-launcher-$(PLATFORM)-$(ARCH)$(EXT) $(BUILD_DIR)/facet-studio-launcher$(EXT)
endif
	@echo "Build complete: $(BUILD_DIR)/facet-studio-launcher$(EXT)"
...
build-launcher-android-arm64:
	@echo "Building facet-studio-launcher for android/arm64..."
	@mkdir -p $(BUILD_DIR)
	@$(MAKE) -C web build-android-arm64 \
		OUTPUT_ANDROID_ARM64="$(CURDIR)/$(BUILD_DIR)/facet-studio-launcher-android-arm64" \
		GO='$(GO)' \
		LDFLAGS='$(LDFLAGS)'
	@echo "Build complete: $(BUILD_DIR)/facet-studio-launcher-android-arm64"
...
build-android-bundle: generate
	...
	@cp $(BUILD_DIR)/$(BINARY_NAME)-android-arm64 $(BUILD_DIR)/android-staging/arm64-v8a/libfacet-studio.so
	@cp $(BUILD_DIR)/facet-studio-launcher-android-arm64 $(BUILD_DIR)/android-staging/arm64-v8a/libfacet-studio-web.so
	@cd $(BUILD_DIR)/android-staging && zip -r ../facet-studio-android-universal.zip .
	@rm -rf $(BUILD_DIR)/android-staging
	@echo "All Android builds complete: $(BUILD_DIR)/facet-studio-android-universal.zip"

## build-pi-zero: Build for Raspberry Pi Zero 2 W (32-bit and 64-bit)
build-pi-zero: build-linux-arm build-linux-arm64
	@echo "Pi Zero 2 W builds: $(BUILD_DIR)/$(BINARY_NAME)-linux-arm (32-bit), $(BUILD_DIR)/$(BINARY_NAME)-linux-arm64 (64-bit)"

## build-all: Build the facet-studio core binary for all Makefile-managed platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install facet-studio to system and copy builtin skills
install: build
...
## uninstall: Remove facet-studio from system
uninstall:
...
## uninstall-all: Remove facet-studio and all data
uninstall-all:
...
vet: generate
	@packages="$$($(GO) list $(GOFLAGS) ./...)" && \
		$(GO) vet $(GOFLAGS) $$(printf '%s\n' "$$packages" | grep -v '^github.com/xibodev/facet-studio/web/')
	@cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
	@$(GO) test $(GOFLAGS) $$($(GO) list $(GOFLAGS) ./... | grep -v github.com/xibodev/facet-studio/web/)
	@cd web && make test
...
## run: Build and run facet-studio
run: build
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build facet-studio-agent facet-studio-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build facet-studio-agent facet-studio-gateway
...
## docker-run: Run facet-studio gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run facet-studio gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run facet-studio agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm facet-studio-agent

## docker-run-agent-full: Run facet-studio agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm facet-studio-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi facet-studio:latest facet-studio:full 2>/dev/null || true


## build-macos-app: Build Facet Studio macOS .app bundle (no terminal window)
build-macos-app:build-launcher
	...
	@echo "macOS .app bundle created: $(BUILD_DIR)/Facet Studio.app"
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.facet-studio/workspace)"
