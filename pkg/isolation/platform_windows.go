...
func createRestrictedPrimaryToken() (windows.Token, error) {
	...
	// CreateRestrictedToken takes NINE parameters:
	//
	//   ExistingTokenHandle, Flags,
	//   DisableSidCount, SidsToDisable,
	//   DeletePrivilegeCount, PrivilegesToDelete,
	//   RestrictedSidCount, SidsToRestrict,
	//   NewTokenHandle
	//
	// This passed TEN -- an extra zero in the SID block pushed &restricted into
	// slot 10, past the end of the signature. The kernel read slot 9 (NULL) as
	// NewTokenHandle and failed with ERROR_NOACCESS (998), so the token was
	// never created and EVERY isolated child on Windows failed to spawn.
	// Isolation has never engaged on this platform.
	//
	// Verified with a standalone probe outside this package: the 9-argument
	// form returns r1=1 with a live handle; the 10-argument form returns r1=0,
	// errno 998, handle 0.
