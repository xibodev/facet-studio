...
func TestResolveInstanceRoot_UsesFacetStudioHome(t *testing.T) {
	// filepath.Clean is separator-correct for the host, so comparing to a
	// POSIX literal only ever matched on POSIX. Build the expectation the way
	// the code does.
	home := filepath.Join("/custom", "facet-studio", "home")
	...
	if root != home {
		t.Fatalf("ResolveInstanceRoot() = %q, want %q", root, home)
	}
}
...
// absFixture builds an absolute path for the host: filepath.IsAbs(absFixture("src")) is
// FALSE on Windows, where an absolute path carries a drive, so POSIX literals
// made these tests assert something the code never claimed.
func absFixture(name string) string {
	if runtime.GOOS == "windows" {
		return filepath.Join("C:"+string(filepath.Separator), name)
	}
	return "/" + name
}

func TestValidateExposePaths(t *testing.T) {
	err := ValidateExposePaths([]config.ExposePath{{Source: absFixture("src"), Target: absFixture("dst"), Mode: "ro"}})
	if err != nil {
		t.Fatalf("ValidateExposePaths() error = %v", err)
	}

	err = ValidateExposePaths([]config.ExposePath{{Source: absFixture("src"), Target: absFixture("dst"), Mode: "bad"}})
	if err == nil {
	...
	err = ValidateExposePaths(
		[]config.ExposePath{
			{Source: absFixture("src"), Target: absFixture("dst"), Mode: "ro"},
			{Source: absFixture("other"), Target: absFixture("dst"), Mode: "rw"},
		},
	)
	if err == nil {
...
func TestMergeExposePaths_OverrideByTarget(t *testing.T) {
	merged := MergeExposePaths(
		[]config.ExposePath{{Source: absFixture("src-a"), Target: absFixture("dst"), Mode: "ro"}},
		[]config.ExposePath{{Source: absFixture("src-b"), Target: absFixture("dst"), Mode: "rw"}},
	)
	if len(merged) != 1 {
		t.Fatalf("MergeExposePaths len = %d, want 1", len(merged))
	}
	if got := merged[0]; got.Source != absFixture("src-b") || got.Target != absFixture("dst") || got.Mode != "rw" {
		t.Fatalf("merged[0] = %+v, want source=%s target=%s mode=rw", got, absFixture("src-b"), absFixture("dst"))
	}
}
...
func TestBuildLinuxMountPlan(t *testing.T) {
	...
	plan := BuildLinuxMountPlan("/rootdir", []config.ExposePath{{Source: absFixture("src"), Target: absFixture("dst"), Mode: "ro"}})
	if len(plan) == 0 {
	...
	for _, rule := range plan {
		...
		if rule.Source == absFixture("src") && rule.Target == absFixture("dst") && rule.Mode == "ro" {
			foundOverride = true
		}
	}
	if !foundRoot {
...
func TestBuildWindowsAccessRules(t *testing.T) {
	...
	for _, rule := range rules {
		if rule.Path == `C:\facet-studio` && rule.Mode == "rw" {
			foundRoot = true
		}
