...
import (
	...
	basechannels "github.com/xibodev/facet-studio/pkg/channels"
...
func facetStudioHomeDir() string {
...
func buildWeixinSyncBufPath(cfg *config.WeixinSettings) string {
	return filepath.Join(facetStudioHomeDir(), "channels", "weixin", "sync", genWeixinAccountKey(cfg)+".json")
}

func buildWeixinContextTokensPath(cfg *config.WeixinSettings) string {
	return filepath.Join(facetStudioHomeDir(), "channels", "weixin", "context-tokens", genWeixinAccountKey(cfg)+".json")
}
