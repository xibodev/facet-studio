...
func (m *Manager) preSend(ctx context.Context, name string, msg bus.OutboundMessage, ch Channel) ([]string, bool) {
	...
	// 1. Stop typing -- but only for a message that ENDS the turn.
	//
	// An auxiliary message (a tool call, a thought, tool feedback) is a
	// progress update, not the answer. Stopping the indicator on the first one
	// left the user staring at a dead screen while the real work ran: measured
	// at 9.4 seconds for a two-step module turn, and a module render takes far
	// longer than that.
	//
	// The turn's own completion still stops typing regardless -- InvokeTypingStop
	// is called by the agent loop on success, error and panic -- so keeping it
	// alive here cannot leave an indicator running after the turn ends.
