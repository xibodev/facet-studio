...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("facet-studio")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@facet-studio hello", want: true},
		{text: "hi @facet-studio:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bfacet-studio\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@facet-studio:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@facet-studio:matrix.org hello", want: "hello"},
		{in: "@facet-studio, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@facet-studio:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@facet-studio:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@facet-studio:matrix.org">Facet Studio</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40facet-studio%3Amatrix.org">Facet Studio</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@facet-studio:matrix.test"), "")
	if err != nil {
