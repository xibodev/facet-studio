// Package deltachat implements a Facet Studio channel for Delta Chat, an
// email-based, end-to-end encrypted messenger.
//
// Facet Studio does not link the Delta Chat core directly. Instead it drives a
...
// wire) that Facet Studio sets when calling send_msg. Viewtype is normally left empty
...
func (c *DeltaChatChannel) createChatmailBootstrapAccount(ctx context.Context, server string) error {
	...
	return fmt.Errorf(
		"deltachat: created chatmail account %q on %s. Update channel_list.deltachat.settings.email from %q to %q, remove the @server bootstrap marker, then run Facet Studio again to use the account",
		addr,
		server,
		c.config.Email,
		addr,
...
func resolveDataDir(configured, channelName string) string {
	...
	return filepath.Join(home, ".facet-studio", "deltachat", name)
}
