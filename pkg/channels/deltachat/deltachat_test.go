...
func TestNewDeltaChatChannel(t *testing.T) {
	...
	t.Run("missing email", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "Next step:") || !strings.Contains(err.Error(), "facet-studio g") {
...
func TestResolveServerPathUsesPATH(t *testing.T) {
	dir := t.TempDir()
	// exec.LookPath only considers a file executable on Windows if it carries
	// one of the PATHEXT extensions, so an extensionless file is invisible to it
	// and the lookup this test exercises could never succeed there.
	name := "deltachat-rpc-server"
	if runtime.GOOS == "windows" {
		name += ".exe"
	}
	fakeServer := filepath.Join(dir, name)
	if err := os.WriteFile(fakeServer, []byte("#!/bin/sh\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("PATH", dir)
...
func TestMentionsBot(t *testing.T) {
	tests := []struct {
		...
		{"multi word display name", "hey Facet Studio Bot, can you help", "Facet Studio Bot", "bot@example.org", true},
		...
		{"display name with prefix word", "hey SuperFacet Studio Bot", "Facet Studio Bot", "bot@example.org", false},
...
func TestResolveDataDir(t *testing.T) {
	...
	home, _ := os.UserHomeDir()
	want := filepath.Join(home, ".facet-studio", "deltachat", "mychan")
...
func TestEnsureAccountCreatesBootstrapAccountAndStops(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "bot123@mehl.cloud") || !strings.Contains(err.Error(), "run Facet Studio again") {
