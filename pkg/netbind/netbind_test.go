...
func TestBuildPlan_ExplicitAnyHostsRemainFamilySpecific(t *testing.T) {
	tests := []struct {
		raw     string
		host    string
		network string
		...
		{raw: "0.0.0.0", host: "0.0.0.0", network: "tcp4"},
		{raw: "::", host: "::", network: "tcp6", v6Only: true},
	}
	for _, tt := range tests {
		t.Run(tt.raw, func(t *testing.T) {
			plan, err := BuildPlan(tt.raw, DefaultLoopback)
			if err != nil {
				t.Fatalf("BuildPlan() error = %v", err)
			}
			if len(plan.groups) != 1 || plan.groups[0].kind != groupExact {
				t.Fatalf("groups = %#v, want one exact binding", plan.groups)
			}
			got := plan.groups[0].exact
			if got.host != tt.host || got.network != tt.network || got.v6Only != tt.v6Only {
				t.Fatalf("binding = %#v, want host=%q network=%q v6Only=%v", got, tt.host, tt.network, tt.v6Only)
			}
			if !IsLoopbackHost(plan.ProbeHost) {
				t.Fatalf("ProbeHost = %q, want loopback", plan.ProbeHost)
			}
		})
	}
}
...
func requirePublicBindOptIn(t *testing.T) {
	t.Helper()
