...
import (
	...
	anthropicmessages "github.com/xibodev/facet-studio/pkg/providers/anthropic_messages"
	"github.com/xibodev/facet-studio/pkg/providers/azure"
	"github.com/xibodev/facet-studio/pkg/providers/bedrock"
	"github.com/xibodev/facet-studio/pkg/providers/common"
)
...
func createClaudeAuthProvider() (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for anthropic. Run: facet-studio auth login --provider anthropic")
	}
...
func createCodexAuthProvider() (LLMProvider, error) {
	...
	if cred == nil {
		return nil, fmt.Errorf("no credentials for openai. Run: facet-studio auth login --provider openai")
	}
...
func CreateProviderFromConfig(cfg *config.ModelConfig) (LLMProvider, string, error) {
	...
	if userAgent == "" {
		userAgent = fmt.Sprintf("Facet Studio/%s", config.Version)
	}
	...
	case "github-copilot":
		...
		if connectMode == "stdio" || connectMode == "grpc" {
			provider, err := NewGitHubCopilotProvider(cfg.APIBase, connectMode, modelID)
			if err != nil {
				return nil, "", err
			}
			return finalizeProviderFromConfig(provider, modelID, cfg)
		}
		provider := NewGitHubCopilotHTTPProvider(cfg.APIKey(), cfg.APIBase, modelID)
		return finalizeProviderFromConfig(provider, modelID, cfg)
