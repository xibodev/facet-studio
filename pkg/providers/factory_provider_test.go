...
func TestBuildModelProviderAliasMap(t *testing.T) {
	for iteration := 0; iteration < 100; iteration++ {
		...
		if got := aliases["vllm"]; got != "vllm" {
			t.Fatalf("iteration %d: vllm normalized to %q", iteration, got)
		}
		if got := aliases["lmstudio"]; got != "lmstudio" {
			t.Fatalf("iteration %d: lmstudio normalized to %q", iteration, got)
		}
	}
...
func TestCreateProviderFromConfig_UserAgent(t *testing.T) {
	defaultUA := "Facet Studio/" + config.Version
