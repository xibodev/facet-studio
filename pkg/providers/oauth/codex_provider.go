...
import (
	...
	orc "github.com/xibodev/facet-studio/pkg/providers/openai_responses_common"
)
...
func resolveCodexModel(model string) (string, string) {
	...
	// gpt-5.4 and higher non-codex models are rejected by ChatGPT Codex endpoint with 400 Bad Request.
	if m == "gpt-5.4" || m == "gpt-5.5" || m == "gpt-5.4-mini" || m == "gpt-5.4-pro" {
		return codexDefaultModel, fmt.Sprintf("model %q is not supported by ChatGPT Codex backend, falling back to %s", model, codexDefaultModel)
	}
...
func CreateCodexTokenSource() func() (string, string, error) {
	return func() (string, string, error) {
		...
		if cred == nil {
			return "", "", fmt.Errorf("no credentials for openai. Run: facet-studio auth login --provider openai")
		}
