...
// summarySeq separates summaries the clock cannot. Windows advances UnixNano
// about every 15ms, so a loop writing several summaries sees one timestamp.
var summarySeq atomic.Uint64

// generateSummaryID must return a value no other summary has, because the id is
// a UNIQUE column: a repeat is not a collision that gets retried, it is an
// insert REFUSED and a summary lost.
//
// It used to be the nanosecond clock alone -- it accepted content and ignored
// it -- so on Windows every summary after the first in a loop was dropped. That
// showed up as a red test that had been written off as pre-existing.
//
// Nothing re-derives an id to look a summary up (one call site, and it keeps
// what it is given), so this does not need to be reproducible. It needs to be
// unique: the counter guarantees that within a process, and the content digest
// separates two processes that write different summaries in the same
// nanosecond.
func generateSummaryID(content string, t time.Time) string {
	sum := sha256.Sum256([]byte(content))
	return fmt.Sprintf("sum_%x_%x_%x", t.UnixNano(), summarySeq.Add(1), sum[:6])
}
