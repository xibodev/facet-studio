...
type turnExecution struct {
	...
	instanceConfigs     map[string]*config.ModelConfig
	requestedSelection  string
	servedTarget        string
	servedIdentity      string
...
func (exec *turnExecution) instanceConfig(candidate providers.FallbackCandidate) (*config.ModelConfig, error) {
	key := candidate.StableKey()
	if modelCfg := exec.instanceConfigs[key]; modelCfg != nil {
		return modelCfg, nil
	}
	modelCfg, err := exec.instanceResolution.ModelConfigForCandidate(candidate)
	if err != nil {
		return nil, err
	}
