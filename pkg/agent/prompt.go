...
const (
	...
	// PromptSourceModuleKnowledge attributes a selected module's overlay and
	// skills, which are module-authored and enter context only on selection.
	PromptSourceModuleKnowledge PromptSourceID = "module:knowledge"
...
type PromptBuildRequest struct {
	...
	// ModuleSummaries are one-line capability summaries for the enabled
	// detached modules, carried per turn so the agent's context reflects what
	// is installed right now rather than what was installed at startup.
	...
	return []PromptSourceDescriptor{
		{
			...
			Owner:           "agent",
			Description:     "Core facet-studio identity and hard rules",
