...
func (p *Pipeline) routeMediaTurn(ts *turnState, exec *turnExecution) error {
	...
	if exec.instanceResolution != nil {
		return fmt.Errorf("media model rerouting is not supported for instance-selected turns; choose an exact target or route whose primary model supports the attached media")
	}
