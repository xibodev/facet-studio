...
func (p *Pipeline) CallLLM(
	...
	callLLM := func(
		...
		if response, handled, streamErr := p.tryInstanceCandidateStreamingLLM(
			providerCtx,
			ts,
			exec,
			messagesForCall,
		...
		runCandidate := func(
			...
			var candidateProvider providers.LLMProvider
			var err error
			if exec.instanceResolution != nil {
				candidateProvider, err = exec.instanceProvider(candidate)
			} else {
				candidateProvider, err = providerForFallbackCandidate(
					ts.agent, exec.activeProvider, exec.activeCandidates, candidate,
				)
			}
			if err != nil {
				return nil, err
			...
			if exec.instanceResolution != nil {
				candidateCfg, err = exec.instanceConfig(candidate)
				if err != nil {
					return nil, err
				}
			} else {
				candidateCfg = resolveActiveModelConfig(
					p.Cfg, ts.agent.Workspace, []providers.FallbackCandidate{candidate},
					candidate.Model, p.Cfg.Agents.Defaults.Provider,
				)
			}
		...
		if len(exec.activeCandidates) > 1 && p.Fallback != nil {
			...
			for _, candidate := range exec.activeCandidates {
				...
				if exec.instanceResolution != nil {
					...
					exec.activeProvider, _ = exec.instanceProvider(candidate)
					exec.activeModelConfig, _ = exec.instanceConfig(candidate)
		...
		response, err := exec.activeProvider.Chat(providerCtx, messagesForCall, toolDefsForCall, exec.llmModel, exec.llmOpts)
		if err == nil && exec.instanceResolution != nil && len(exec.activeCandidates) > 0 {
			exec.servedTarget = exec.activeCandidates[0].DisplayName
			exec.servedIdentity = exec.activeCandidates[0].StableKey()
		}
		return response, err
	}
...
func (p *Pipeline) applyBeforeLLMModelRewrite(ts *turnState, exec *turnExecution) error {
	if exec.instanceResolution != nil {
		return fmt.Errorf("hook model rewrites are not supported for instance-selected turns; select an exact target or route before the turn starts")
	}
