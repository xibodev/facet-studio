...
func TestProcessHook_HelperProcess(t *testing.T) {
	if os.Getenv("FACET_STUDIO_HOOK_HELPER") != "1" {
		return
	}
...
func TestAgentLoop_MountProcessHook_IsolationSupportsRelativeDirAndCommand(t *testing.T) {
	...
	root := t.TempDir()
	t.Setenv(config.EnvHome, filepath.Join(root, "facet-studio-home"))
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"FACET_STUDIO_HOOK_HELPER=1",
		"FACET_STUDIO_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "FACET_STUDIO_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("FACET_STUDIO_HOOK_MODE")
	eventLog := os.Getenv("FACET_STUDIO_HOOK_EVENT_LOG")
