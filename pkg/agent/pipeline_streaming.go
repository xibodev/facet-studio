...
func (p *Pipeline) tryInstanceCandidateStreamingLLM(
	ctx context.Context,
	ts *turnState,
	exec *turnExecution,
	...
	if exec == nil || exec.instanceResolution == nil || len(exec.activeCandidates) < 2 {
		return nil, false, nil
	}
	if !p.instanceCandidateStreamingEligible(ts, exec) {
		return nil, false, nil
	}
	...
	result, err := p.Fallback.ExecuteCandidate(ctx, exec.activeCandidates, func(
		ctx context.Context,
		candidate providers.FallbackCandidate,
	) (*providers.LLMResponse, error) {
		provider, err := exec.instanceProvider(candidate)
		if err != nil {
			return nil, err
		}
		...
		if !supportsStreaming || !modelCfg.Streaming.Enabled {
			return provider.Chat(ctx, messages, tools, candidate.Model, exec.llmOpts)
		}
		candidateOpts := shallowCloneLLMOptions(exec.llmOpts)
		delete(candidateOpts, "thinking_level")
		candidateThinking := thinkingSettingsFromModelConfig(modelCfg)
		...
		if streamErr != nil {
			if publisher.Published() || publisher.ReasoningPublished() {
				...
				return nil, &providers.FailoverError{
					Reason: providers.FailoverFormat, Provider: candidate.Provider,
					Model: candidate.Model, Wrapped: configuredStreamingVisibleError{err: streamErr},
				}
			}
	...
	for _, candidate := range exec.activeCandidates {
		if candidate.StableKey() != result.IdentityKey {
			continue
		}
...
func (p *Pipeline) instanceCandidateStreamingEligible(ts *turnState, exec *turnExecution) bool {
	if p == nil || p.Bus == nil || p.Fallback == nil || ts == nil || exec == nil {
		return false
	}
