...
func setupWorkspace(t *testing.T, files map[string]string) string {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "facet-studio-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSingleSystemMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sys := msgs[0].Content
			if !strings.Contains(sys, "facet-studio") {
...
// (~/.facet-studio/skills) invalidates the cached system prompt.
func TestGlobalSkillFileContentChange(t *testing.T) {
	// FACET_STUDIO_HOME, not HOME: config.GetHome reads that override first and
	// otherwise calls os.UserHomeDir, which on Windows reads USERPROFILE and
	// ignores HOME entirely. Setting HOME isolated nothing there, so the loader
	// read the developer's real home, found no global-skill, and the test failed
	// on a machine where the code was working.
	//
	// The override also removes the ".facet-studio" path segment, because
	// GetHome appends that only when deriving from the user's home directory.
	tmpHome := t.TempDir()
...
func TestBuiltinSkillFileContentChange(t *testing.T) {
	// See TestGlobalSkillFileContentChange: HOME does not isolate on Windows.
	tmpHome := t.TempDir()
	t.Setenv(config.EnvHome, tmpHome)
	...
	t.Setenv("FACET_STUDIO_BUILTIN_SKILLS", builtinRoot)
	...
	for g := range goroutines {
		...
		go func(id int) {
			...
			for i := range iterations {
				...
				if !strings.Contains(result, "facet-studio") {
...
func BenchmarkBuildMessagesWithCache(b *testing.B) {
	tmpDir, _ := os.MkdirTemp("", "facet-studio-bench-*")
	defer os.RemoveAll(tmpDir)
