...
func (al *AgentLoop) processMessage(ctx context.Context, msg bus.InboundMessage) (string, error) {
	...
	if selection := strings.TrimSpace(msg.Context.Raw[bus.MetadataKeyModelSelection]); selection != "" {
		if al.instanceSelectionResolver == nil {
			return "", fmt.Errorf("model selection %q is unavailable: instance resolver is not configured", selection)
		}
		resolved, err := al.instanceSelectionResolver(ctx, selection)
		if err != nil {
			return "", fmt.Errorf("resolve model selection %q: %w", selection, err)
		}
