...
func (p *Pipeline) SetupTurn(ctx context.Context, ts *turnState) (*turnExecution, error) {
	...
	if ts.opts.InstanceResolution != nil {
		if len(ts.opts.InstanceResolution.Candidates) == 0 {
			return nil, fmt.Errorf("instance selection %q resolved no candidates", ts.opts.RequestedSelection)
		}
		exec.instanceResolution = ts.opts.InstanceResolution
		exec.instanceProviders = make(map[string]providers.LLMProvider)
		exec.instanceConfigs = make(map[string]*config.ModelConfig)
		...
		var err error
		activeProvider, err = exec.instanceProvider(primary)
		if err != nil {
			return nil, fmt.Errorf("initialize selected target %q: %w", primary.DisplayName, err)
		}
	...
	if exec.instanceResolution != nil {
		exec.activeModelConfig, _ = exec.instanceConfig(activeCandidates[0])
	} else {
		exec.activeModelConfig = resolveActiveModelConfig(
			p.Cfg, ts.agent.Workspace, activeCandidates, activeModel,
