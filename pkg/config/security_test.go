...
func TestSecurityPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "standard path",
			configDir: filepath.Join("/home/user/.facet-studio", "config.json"),
			want:      filepath.Join("/home/user/.facet-studio", SecurityConfigFile),
		},
		{
			name:      "nested path",
...
`
	...
	t.Run("test for env overwrite", func(t *testing.T) {
		...
		t.Setenv("FACET_STUDIO_CHANNELS_QQ_APP_SECRET", "qq_app_secret_env")
		t.Setenv("FACET_STUDIO_TOOLS_WEB_BRAVE_API_KEYS", "brave_key_env,abc")
