...
type GatewayConfig struct {
	Host      string `json:"host"                env:"FACET_STUDIO_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"FACET_STUDIO_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"FACET_STUDIO_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"FACET_STUDIO_LOG_LEVEL"`
}
...
// The FACET_STUDIO_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("FACET_STUDIO_LOG_LEVEL"); envLevel != "" {
