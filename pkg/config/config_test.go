  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "facet-studio-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".facet-studio", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithFacetStudioHome(t *testing.T) {
	t.Setenv("FACET_STUDIO_HOME", "/custom/facet-studio/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/facet-studio/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf(
			"Workspace path with FACET_STUDIO_HOME = %q, want %q",
...
func TestLoadConfig_WarnsForPlaintextAPIKey(t *testing.T) {
	...
	t.Setenv("FACET_STUDIO_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("FACET_STUDIO_SSH_KEY_PATH", "")
...
// unchanged when FACET_STUDIO_KEY_PASSPHRASE is not set.
func TestLoadConfig_NoSealWithoutPassphrase(t *testing.T) {
	...
	t.Setenv("FACET_STUDIO_KEY_PASSPHRASE", "")
	t.Setenv("FACET_STUDIO_SSH_KEY_PATH", "")
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when FACET_STUDIO_KEY_PASSPHRASE
