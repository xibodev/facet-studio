...
```bash
chmod 600 ~/.facet-studio/.security.yml
```
...
```bash
cp ~/.facet-studio/config.json ~/.facet-studio/config.json.backup
```

## Step 2: Create .security.yml
```bash
cp security.example.yml ~/.facet-studio/.security.yml
```

## Step 3: Fill in your API keys
Edit ~/.facet-studio/.security.yml and replace placeholders with your actual keys.

## Step 4: Simplify config.json (Recommended)
Remove sensitive fields from ~/.facet-studio/config.json:
...
```bash
rm ~/.facet-studio/config.json.backup
```
...
3. Set file permissions: chmod 600 ~/.facet-studio/.security.yml
...
```bash
# Channels
export FACET_STUDIO_CHANNELS_TELEGRAM_TOKEN="token-from-env"
export FACET_STUDIO_CHANNELS_DISCORD_TOKEN="discord-token-from-env"

# Web Tools
export FACET_STUDIO_TOOLS_WEB_BRAVE_API_KEY="brave-key-from-env"
export FACET_STUDIO_TOOLS_WEB_BAIDU_API_KEY="baidu-key-from-env"

# Skills
export FACET_STUDIO_TOOLS_SKILLS_GITHUB_TOKEN="github-token-from-env"
```
...
- Verify the file permissions allow reading (chmod 600 ~/.facet-studio/.security.yml)
