...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.facet-studio/workspace" {
		t.Errorf("expected workspace '~/.facet-studio/workspace', got '%s'", workspace)
	}
...
func TestConvertToFacetStudio(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToFacetStudio("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.facet-studio/workspace" {
		t.Errorf("expected workspace '~/.facet-studio/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToFacetStudioWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToFacetStudio("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToFacetStudioWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToFacetStudioWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.facet-studio/workspace" {
		t.Errorf("expected workspace '~/.facet-studio/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
