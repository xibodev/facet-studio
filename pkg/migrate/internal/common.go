...
func ExpandHome(path string) string {
	...
	if path[0] == '~' {
		...
		if len(path) > 1 && (path[1] == '/' || path[1] == filepath.Separator) {
			// filepath.Join, not string concatenation: `home + path[1:]` kept
			// the caller's forward slash, producing "C:\Users\me/openclaw" on
			// Windows. Go accepts that, so it never broke anything -- it just
			// meant every path derived from a tilde carried mixed separators
			// and compared unequal to the same path built properly.
