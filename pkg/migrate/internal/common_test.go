...
func TestExpandHomeWithTilde(t *testing.T) {
	...
	// home+"/path" was asserting the BUG: ExpandHome concatenated the caller's
	// forward slash, so a Windows result was "C:\Users\me/path" -- accepted by
	// Go, but unequal to the same path built with filepath.Join, which is how
	// every other caller builds one.
...
// Both of these compared against POSIX literals while the functions use
// filepath.Join and filepath.Rel, which are separator-correct for the host --
// so they could only ever match on POSIX. Build the expectations the same way
// the code does.
func TestResolveWorkspace(t *testing.T) {
	home := filepath.Join("/home", "user", ".facet-studio")
...
func TestRelPath(t *testing.T) {
	...
	result := RelPath(filepath.Join(home, "workspace", "file.txt"), home)
...
func TestResolveTargetHome(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, filepath.Join(home, ".facet-studio"), result)
}
