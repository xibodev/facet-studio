...
func TestAudioModelTranscriberTranscribe(t *testing.T) {
	...
	t.Run("unsupported audio format", func(t *testing.T) {
		...
		// fmt.Sprintf("%q") escapes the backslashes in a Windows path, so
		// building the expectation by concatenation only ever matched on
		// POSIX. Quote it the same way the message does.
