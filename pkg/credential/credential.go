...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via FACET_STUDIO_KEY_PASSPHRASE
...
//  2. FACET_STUDIO_SSH_KEY_PATH env var
//  3. ~/.ssh/facet-studio_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "FACET_STUDIO_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading FACET_STUDIO_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "FACET_STUDIO_SSH_KEY_PATH"

// facetStudioHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const facetStudioHome = "FACET_STUDIO_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "facet-studio-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (FACET_STUDIO_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// FACET_STUDIO_SSH_KEY_PATH env var or ~/.ssh/facet-studio_ed25519.key.
...
//   - exact match with FACET_STUDIO_SSH_KEY_PATH env var
//   - within the FACET_STUDIO_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with FACET_STUDIO_SSH_KEY_PATH.
	...
	// Within FACET_STUDIO_HOME.
	if picoHome := os.Getenv(facetStudioHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="facet-studio-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set FACET_STUDIO_SSH_KEY_PATH or place key at ~/.ssh/facet-studio_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (FACET_STUDIO_SSH_KEY_PATH, FACET_STUDIO_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. FACET_STUDIO_SSH_KEY_PATH env var
//  3. ~/.ssh/facet-studio_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the facet-studio-specific SSH key path if it exists.
