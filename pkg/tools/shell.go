...
import (
	...
	"github.com/xibodev/facet-studio/pkg/constants"
	"github.com/xibodev/facet-studio/pkg/isolation"
...
var (
	...
	//
	// The Windows alternative allows backslashes WITHIN the path. It used to be
	// `[A-Za-z]:\\[^\\"']+`, which stops at the second backslash, so
	// "C:\Users\me\ws\f.go" matched only "C:\Users" -- a drive-root prefix that
	// is by definition above any workspace. The real path was never validated
...
func (t *ExecTool) executeRun(ctx context.Context, args map[string]any) *ToolResult {
	...
	if t.restrictToWorkspace && t.workingDir != "" && cwd != t.workingDir {
		resolved, err := pathlink.Resolve(cwd)
		if err != nil {
		...
		if isAllowedPath(resolved, t.allowedPathPatterns) {
			...
			wsResolved, _ := pathlink.Resolve(absWorkspace)
			if wsResolved == "" {
...
func (t *ExecTool) guardCommand(command, cwd string) string {
	...
	if t.restrictToWorkspace {
		...
		for _, loc := range matchIndices {
			...
			// "file://C:\ws\f.txt" matches the POSIX alternative starting at
			// the double slash, giving "//C:\ws\f.txt". The old pattern stopped
			// at the second backslash and never exposed this; widening it did.
			// A drive letter after the slashes means it is a Windows path
			// wrapped in a URI, so unwrap it rather than judge "//C:" as a
			// directory that is not there.
			if unwrapped, ok := unwrapURIDrivePath(pathText); ok {
				pathText = unwrapped
			}

			// The kernel pseudo-device allowlist is keyed by the LITERAL path,
			// so it must be consulted before the path is made absolute. It used
			// to be checked after, against a joined path that could never match
			// a "/dev/null" key -- dead code on Windows, where these were then
			// allowed by accident rather than by intent.
			if safePaths[path.Clean(filepath.ToSlash(pathText))] {
				continue
			}
			...
			// Check symlinks AND junctions. This comment used to sit above a
			// bare EvalSymlinks, which does not see a junction at all -- it
			// stated a guarantee the code did not provide.
			if resolved, err := pathlink.Resolve(p); err == nil {
				p = resolved
			}
			...
			if err != nil {
				// Rel fails when the two paths cannot be compared at all --
				// on Windows, when they are on different drives. That is not a
				// reason to allow the path; it is proof it is not under the
				// working directory. This used to `continue`, which is ALLOW,
				// leaving every other-drive path unguarded.
...
func commandPathAbs(pathText, cwdPath string) (string, error) {
	...
	// A POSIX-absolute path is NOT absolute to filepath on Windows -- an
	// absolute path there carries a drive -- so "/etc/passwd" used to be joined
	// under the working directory and judged safely inside it. The shell does
	// not agree: PowerShell resolves a leading slash against the current drive
	// root, so `ls /tmp` reads C:\tmp, outside the workspace entirely.
	//
	// This was demonstrated rather than theorised: a test run printed a real
	// directory listing of C:\tmp while the guard reported the path as inside.
	if runtime.GOOS == "windows" && strings.HasPrefix(pathText, "/") {
		vol := filepath.VolumeName(cwdPath)
		if vol == "" {
			vol = filepath.VolumeName(mustAbs(cwdPath))
		}
		return filepath.Abs(vol + filepath.FromSlash(pathText))
	}
	return filepath.Abs(filepath.Join(cwdPath, pathText))
}

// mustAbs resolves a path, falling back to the input when it cannot.
...
// after the scheme) into "C:\ws\f.txt", reporting whether it did.
//
// It unwraps ONLY when a drive letter follows the slashes. "//etc/passwd" is a
// real path on a POSIX-shaped command line and must stay exactly as it is --
// TestShellTool_URLBypassPrevented exists because a command can smuggle one
// after a URL containing the same substring.
func unwrapURIDrivePath(p string) (string, bool) {
