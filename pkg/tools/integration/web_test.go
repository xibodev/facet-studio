...
// with cf-mitigated: challenge triggers a retry using the honest facet-studio User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest facet-studio user agent
	if !strings.Contains(receivedUAs[1], "facet-studio") {
		t.Errorf("retry request UA = %q, want it to contain 'facet-studio'", receivedUAs[1])
	}
}
