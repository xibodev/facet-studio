...
func TestHostRW_Read_PermissionDenied(t *testing.T) {
	...
	// Windows has no POSIX permission bits: os.WriteFile with 0o000 grants no
	// protection there, the read succeeds, and the assertion below then
	// dereferences a nil error. This tested the OS, not the code.
	if runtime.GOOS == "windows" {
		t.Skip("no POSIX permission bits on Windows; 0o000 does not deny access")
	}
	tmpDir := t.TempDir()
