...
func validatePathWithAllowPaths(
	...
	if restrict {
		...
		if resolved, err = pathlink.Resolve(absWorkspace); err == nil {
			workspaceReal = resolved
		}

		// pathlink.Resolve, not EvalSymlinks: a Windows junction is invisible
		// to EvalSymlinks, and needs no privilege to create, so the boundary
		// was advisory on the platform where it is easiest to cross.
		if resolved, err = pathlink.Resolve(absPath); err == nil {
...
func resolveExistingAncestor(path string) (string, error) {
	for current := filepath.Clean(path); ; current = filepath.Dir(current) {
		if resolved, err := pathlink.Resolve(current); err == nil {
			return resolved, nil
		} else if !os.IsNotExist(err) {
			return "", err
		}
...
func resolvePathAgainstExistingAncestor(path string) (string, error) {
	...
	for current := cleaned; ; current = filepath.Dir(current) {
		resolved, err := pathlink.Resolve(current)
		if err == nil {
...
func (t *WriteFileTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	if !overwrite {
		// Close it. This is an existence check, not a read, and it used to
		// discard the *fs.File -- leaking a handle on the SUCCESS path, i.e.
		// every time a user is correctly told the file already exists. The two
		// sibling call sites both defer a Close; this one did not.
		//
		// Windows surfaces the leak as a directory that cannot be removed while
		// every assertion passes; Linux hides it entirely.
