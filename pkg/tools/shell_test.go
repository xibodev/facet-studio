...
func TestShellTool_ExitCodeDetails(t *testing.T) {
	...
	args := map[string]any{
		"action": "run",
		// Exercise the shell's own exit code. Windows PowerShell maps a nested
		// native command's failure to 1 unless explicitly told to propagate it.
		"command": "exit 42",
	}
...
func TestShellTool_TimeoutWithPartialOutput(t *testing.T) {
	...
	// Allow the native shell to start even under full-suite contention.
	...
	if runtime.GOOS == "windows" {
		command = "Write-Output 'partial output before timeout'; Start-Sleep -Seconds 30"
	}
	args := map[string]any{
		"action":  "run",
		"command": command,
	...
	require.Contains(t, result.ForLLM, "Partial output before timeout:")
	require.Contains(t, result.ForLLM, "partial output before timeout")
...
func TestShellTool_URLsNotBlocked(t *testing.T) {
	...
	commands := []string{
		...
		"git clone https://github.com/xibodev/facetstudio.git",
	}
...
// Background shells may take longer to start than a fixed sleep permits,
// especially Windows PowerShell under a full-suite workload. Keep exercising
// the public read action while waiting for the expected output under a bound.
func waitForShellOutput(t *testing.T, tool *ExecTool, ctx context.Context, sessionID, want string) ExecResponse {
	t.Helper()
	var response ExecResponse
	var output strings.Builder
	require.Eventually(t, func() bool {
		result := tool.Execute(ctx, map[string]any{"action": "read", "sessionId": sessionID})
	...
	}, 10*time.Second, 50*time.Millisecond, "session did not produce %q", want)
...
func TestShellTool_Write_Read_NonPTY(t *testing.T) {
	...
	// PowerShell's cat is Get-Content, not a stdin echo utility.
	command := "cat"
	if runtime.GOOS == "windows" {
		command = "while ($null -ne ($line = [Console]::ReadLine())) { [Console]::WriteLine($line) }"
	}
	...
	readResp := waitForShellOutput(t, tool, ctx, resp.SessionID, "hello world")
...
func TestShellTool_Read_NonPTY_Running(t *testing.T) {
	...
	// Use the host shell directly and keep it alive after producing output.
	command := "echo line1; sleep 0.5; echo line2; sleep 0.5; echo line3; sleep 30"
	if runtime.GOOS == "windows" {
		command = "Write-Output line1; Start-Sleep -Milliseconds 500; Write-Output line2; Start-Sleep -Milliseconds 500; Write-Output line3; Start-Sleep -Seconds 30"
	}
	result := tool.Execute(ctx, map[string]any{
		"action":     "run",
		"command":    command,
	...
	readResp := waitForShellOutput(t, tool, ctx, resp.SessionID, "line1")
	require.Contains(t, readResp.Output, "line1")
	require.Equal(t, "running", readResp.Status)
	if !strings.Contains(readResp.Output, "line3") {
		readResp = waitForShellOutput(t, tool, ctx, resp.SessionID, "line3")
	}
...
func TestShellTool_Poll_Status(t *testing.T) {
	...
	}, 10*time.Second, 50*time.Millisecond, "background command did not finish")
	require.Equal(t, "done", pollResp.Status)
}
...
func TestShellTool_CustomAllowDoesNotBypassDenyPatterns(t *testing.T) {
	...
	got := tool.guardCommand(`jq -n '$ENV.FACET_STUDIO_VARIANT_CANARY'`, t.TempDir())
