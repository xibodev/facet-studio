...
// setupService builds a service over a store in the OS temp directory.
//
// The path used to be RELATIVE, so every run wrote its store into the package
// source directory. Callers defer os.Remove, but a test that times out never
// runs its defers -- and this package has a test that takes minutes, so the
// tree collected stray test_cron_*.json files. os.CreateTemp also removes the
// UnixNano name, which on Windows (~15ms granularity) could collide.
func setupService(handler JobHandler) (*CronService, string) {
	f, err := os.CreateTemp("", "test_cron_*.json")
	if err != nil {
		panic("cron test: cannot create a temp store: " + err.Error())
	}
	path := f.Name()
	_ = f.Close()
	// The service creates the file itself; an existing empty file would be
	// parsed as a corrupt store.
	_ = os.Remove(path)
	return NewCronService(path, handler), path
}
