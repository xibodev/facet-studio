...
// MetadataKeySelectedModule carries the module a user pointed the agent at for
// one turn -- the connector gesture.
//
// It travels beside the message rather than inside it because the two readers
// need different things: the model is TOLD in a sentence it can see in the
// transcript, and the host needs a value it can match against an installed
// module id to decide whose overlay and skills to load. Recovering the id by
// parsing that sentence would break the moment the wording changed.
const MetadataKeySelectedModule = "selected_module"

const (
	MetadataKeyModelSelection = "model_selection"
	MetadataKeyServedTarget   = "served_target"
	MetadataKeyServedIdentity = "served_identity"
)
