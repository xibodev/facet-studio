...
import (
	...
	ppid "github.com/xibodev/facet-studio/pkg/pid"
...
func startGatewayLikeProcess(t *testing.T) *exec.Cmd {
	...
	cmd = exec.Command("sh", "-c", "sleep 30 # facet-studio gateway")
...
func writeTestPidFile(t *testing.T, data ppid.PidFileData) string {
	...
	path := filepath.Join(globalConfigDir(), ".facetstudio.pid")
...
func resetGatewayTestState(t *testing.T) {
	...
	t.Setenv("FACET_STUDIO_HOME", t.TempDir())
	t.Cleanup(func() {
...
func TestLooksLikeGatewayCommandLine(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "default facet-studio gateway",
			cmdline: "/usr/local/bin/facet-studio gateway -E",
			want:    true,
		},
		{
...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-facet-studio")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("FACET_STUDIO_BINARY", invalidBinaryPath)
...
func TestFindFacetStudioBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "facet-studio-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("FACET_STUDIO_BINARY", mockBinary)

	got := utils.FindFacetStudioBinary()
	if got != mockBinary {
		t.Errorf("FindFacetStudioBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindFacetStudioBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When FACET_STUDIO_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("FACET_STUDIO_BINARY", "/nonexistent/facet-studio-binary")

	got := utils.FindFacetStudioBinary()
	// Should not return the invalid path; falls back to "facet-studio" or another found path
	if got == "/nonexistent/facet-studio-binary" {
		t.Errorf("FindFacetStudioBinary() returned invalid env path %q, expected fallback", got)
	}
}
