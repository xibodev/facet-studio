...
var gateway = struct {
	...
	logs                *LogBuffer
	pidData             *ppid.PidFileData // pid file data read from facetstudio.pid.json
...
// isLikelyGatewayProcess returns whether PID appears to be a facet-studio gateway
...
func isLikelyGatewayProcess(pid int) (bool, bool) {
	...
	if runtime.GOOS == "windows" {
		...
		if strings.HasPrefix(line, "\"") {
			// EVERY NAME THE HARNESS SHIPS UNDER. It is `facet-studio-kernel`
			// beside the shell in a full-product install, `facet-studio` when
			// built standalone, and `facetstudio` for older installs. Matching
			// only one name reports "not running" for a process that IS
			// running, and the shell then starts a second one.
			for _, n := range []string{
				"\"facet-studio-kernel.exe\"",
				"\"facet-studio.exe\"",
				"\"facetstudio.exe\"",
			} {
...
// represents "facet-studio gateway ..." regardless of executable filename.
...
func (h *Handler) validateGatewayPidData(
	...
	if gatewayProcess, inspected := gatewayProcessMatcher(pidData.PID); inspected {
		if !gatewayProcess {
			return false, true, "pid process command is not facet-studio gateway"
		}
		return true, true, ""
	}
...
func (h *Handler) startGatewayLocked(initialStatus string, existingPid int) (int, error) {
	...
	// Start new process
	// Locate the facet-studio executable
	execPath := utils.FindFacetStudioBinary()
	...
	pid = cmd.Process.Pid
	logger.InfoC("gateway", fmt.Sprintf("Started facet-studio gateway (PID: %d) from %s", pid, execPath))
...
// handleGatewayStart starts the facet-studio gateway subprocess.
//
