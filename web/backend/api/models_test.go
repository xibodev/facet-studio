...
func resetModelProbeHooks(t *testing.T) {
	...
	origCopilotProbe := probeGitHubCopilotFunc
	...
	t.Cleanup(func() {
		...
		probeGitHubCopilotFunc = origCopilotProbe
...
func TestHandleListModels_AvailabilityUsesRuntimeProbesForLocalModels(t *testing.T) {
	...
	probeGitHubCopilotFunc = func() bool {
		mu.Lock()
		copilotProbes++
		mu.Unlock()
		return true
	...
	if len(tcpProbes) != 0 {
		t.Fatalf("tcp probes = %#v, want none for native Copilot", tcpProbes)
	}
	if copilotProbes != 1 {
		t.Fatalf("native Copilot probes = %d, want 1", copilotProbes)
	}
}
...
func TestHandleListModels_ReturnsProviderOptionsWithoutPersistingLegacyMigration(t *testing.T) {
	...
	if option, ok := optionsByID["github-copilot"]; !ok {
		...
		t.Fatalf("github-copilot default_api_base = %q, want empty native endpoint", option.DefaultAPIBase)
...
func TestHandleFetchModels_ModelIndexUsesStoredKey(t *testing.T) {
	...
	tmp := t.TempDir()
	oldHome := os.Getenv("FACET_STUDIO_HOME")
	t.Setenv("FACET_STUDIO_HOME", filepath.Join(tmp, ".facet-studio"))
	defer func() {
		if oldHome != "" {
			os.Setenv("FACET_STUDIO_HOME", oldHome)
		} else {
			os.Unsetenv("FACET_STUDIO_HOME")
		}
	}()
