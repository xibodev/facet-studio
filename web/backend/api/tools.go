...
import (
	...
	picotools "github.com/xibodev/facet-studio/pkg/tools"
)
...
func (h *Handler) handleListTools(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(toolSupportResponse{
		Tools: append(buildToolSupport(cfg), moduleToolSupport(r.Context())...),
	})
}

// moduleToolSupport lists the tools installed modules contribute.
//
// The catalog above is what is compiled in, read from config. Module tools are
// discovered at runtime and appear in neither -- so this page showed twenty
// tools while the agent actually had thirty, and a person inspecting the app
// could not see that a module contributed anything callable.
//
// There is no per-tool config key: a module's capabilities are available
// exactly when the module is ENABLED, which the Modules page controls.
//
// Status used to be the literal "enabled" for every module tool, on the
// reasoning that installing a module was the only switch there was. Disabling
// added a second one, and this page kept reporting the old answer -- so a
// disabled module still listed six callable tools that the agent did not have.
//
// A second implementation of "what is available" disagreeing with the first is
// the failure this codebase keeps finding. This one now reads the same
// Disabled flag the agent's registration does.
func moduleToolSupport(ctx context.Context) []toolSupportItem {
	var items []toolSupportItem
	for _, in := range moduletools.Discover(ctx, config.GetHome()) {
		...
		for _, c := range in.Descriptor.Capabilities {
			items = append(items, toolSupportItem{
				Name:        moduletools.ToolName(in.Descriptor.Module, c.ID),
				Description: c.Summary,
				Category:    "module: " + in.Descriptor.Name,
				Status:      status,
			})
		}
	}
