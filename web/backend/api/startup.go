...
const (
	autoStartEntryName = "FacetStudioLauncher"
	launchAgentLabel   = "io.facetstudio.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "facet-studio-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Type=Application",
			"Version=1.0",
			"Name=Facet Studio Web",
			"Comment=Start Facet Studio Web on login",
