...
var (
	...
	findFacetStudioBinaryForInfo = resolveGatewayBinaryForVersionInfo
	runFacetStudioVersionOutput  = executeFacetStudioVersion
	...
	versionLinePattern           = regexp.MustCompile(
		`^(?:[^A-Za-z0-9]*\s*)?facet-studio(?:\.exe)?\s+([^\s(]+)` +
			`(?:\s+\(git:\s*([^)]+)\))?\s*$`,
	)
)
...
// resolveSystemVersionInfo prefers the actual facet-studio binary version output,
...
func (h *Handler) resolveSystemVersionInfoUncached(ctx context.Context) systemVersionResponse {
	...
	execPath := strings.TrimSpace(findFacetStudioBinaryForInfo())
	if execPath == "" {
		return fallback
	}
	...
	output, err := runFacetStudioVersionOutput(cmdCtx, execPath)
	if err != nil {
		return fallback
	}

	parsed, ok := parseFacetStudioVersionOutput(output)
	if !ok {
		return fallback
	}
...
func resolveGatewayBinaryForVersionInfo() string {
	...
	return utils.FindFacetStudioBinary()
}
...
// executeFacetStudioVersion runs the version subcommand against the
// discovered facet-studio executable.
func executeFacetStudioVersion(ctx context.Context, execPath string) (string, error) {
...
// parseFacetStudioVersionOutput extracts version/build/go fields from CLI output.
// It accepts banner/ANSI-decorated output and only requires the version line.
func parseFacetStudioVersionOutput(raw string) (systemVersionResponse, bool) {
