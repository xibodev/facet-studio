...
func setupVersionTestIsolation(t *testing.T) {
	...
	originalFinder := findFacetStudioBinaryForInfo
	originalRunner := runFacetStudioVersionOutput
	originalFallback := launcherBuildInfoForVersion
	t.Cleanup(func() {
		currentGatewayVersionState = originalGatewayState
		findFacetStudioBinaryForInfo = originalFinder
		runFacetStudioVersionOutput = originalRunner
...
func TestGetSystemVersionUsesFacetStudioBinaryInfo(t *testing.T) {
	...
	findFacetStudioBinaryForInfo = func() string { return "facet-studio" }
	runFacetStudioVersionOutput = func(_ context.Context, _ string) (string, error) {
		return "🦞 facet-studio v1.2.3 (git: deadbeef)\n  Build: 2026-03-27T12:34:56Z\n  Go: go1.25.8\n", nil
	}
...
func TestParseFacetStudioVersionOutput(t *testing.T) {
	setupVersionTestIsolation(t)

	raw := "\u001b[1;31m████\u001b[0m\n🦞 facet-studio 18ec263 (git: 18ec2631)\n  Build: 2026-03-27T10:43:34+0000\n  Go: go1.25.8\n"
	got, ok := parseFacetStudioVersionOutput(raw)
	if !ok {
		t.Fatal("parseFacetStudioVersionOutput() should parse valid output")
	}
...
func TestParseFacetStudioVersionOutputIgnoresUsageLine(t *testing.T) {
	setupVersionTestIsolation(t)

	raw := "Usage: facet-studio version [flags]\n"
	got, ok := parseFacetStudioVersionOutput(raw)
	if ok {
		t.Fatalf("parseFacetStudioVersionOutput() parsed usage line unexpectedly: %#v", got)
	}
}

func TestParseFacetStudioVersionOutputAcceptsLetterOnlyHashVersion(t *testing.T) {
	setupVersionTestIsolation(t)

	raw := "facet-studio abcdefa (git: abcdefabcdefabcdefabcdefabcdefabcdefabcd)\n"
	got, ok := parseFacetStudioVersionOutput(raw)
	if !ok {
		t.Fatal("parseFacetStudioVersionOutput() should parse letter-only hash version")
	}
...
func TestResolveSystemVersionInfoCachesWhileGatewayAlive(t *testing.T) {
	...
	runFacetStudioVersionOutput = func(_ context.Context, _ string) (string, error) {
		runCount++
		return fmt.Sprintf("facet-studio v1.2.%d\n", runCount), nil
	}
...
func TestResolveSystemVersionInfoInvalidatesCacheWhenGatewayStops(t *testing.T) {
	...
	runFacetStudioVersionOutput = func(_ context.Context, _ string) (string, error) {
		runCount++
		return fmt.Sprintf("facet-studio v2.0.%d\n", runCount), nil
	}
...
func TestResolveGatewayBinaryForVersionInfoPrefersGatewayCommandPath(t *testing.T) {
	...
	gateway.cmd = &exec.Cmd{Path: "/tmp/facet-studio-from-gateway"}
	gateway.mu.Unlock()
	t.Cleanup(func() {
		gateway.mu.Lock()
	...
	if got != "/tmp/facet-studio-from-gateway" {
		t.Fatalf("exec path = %q, want %q", got, "/tmp/facet-studio-from-gateway")
	}
}
