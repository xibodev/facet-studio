...
func setupOAuthTestEnv(t *testing.T) (string, func()) {
	...
	oldPicoHome := os.Getenv("FACET_STUDIO_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("FACET_STUDIO_HOME", filepath.Join(tmp, ".facet-studio")); err != nil {
		t.Fatalf("set FACET_STUDIO_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldPicoHome == "" {
			_ = os.Unsetenv("FACET_STUDIO_HOME")
		} else {
			_ = os.Setenv("FACET_STUDIO_HOME", oldPicoHome)
		}
	}
