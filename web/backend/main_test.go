...
func TestBindLauncherHomeToExplicitConfig(t *testing.T) {
	t.Run("binds absent home to config directory", func(t *testing.T) {
		t.Setenv(config.EnvHome, "")
		dir := t.TempDir()
		if err := bindLauncherHomeToExplicitConfig(filepath.Join(dir, "config.json")); err != nil {
			t.Fatal(err)
		}
		if got := os.Getenv(config.EnvHome); got != dir {
			t.Fatalf("home=%q want=%q", got, dir)
		}
	})
	t.Run("explicit environment home wins", func(t *testing.T) {
		want := t.TempDir()
		t.Setenv(config.EnvHome, want)
		if err := bindLauncherHomeToExplicitConfig(filepath.Join(t.TempDir(), "config.json")); err != nil {
			t.Fatal(err)
		}
		if got := os.Getenv(config.EnvHome); got != want {
...
func TestOpenLauncherListeners_HonorsIPv6OnlyHost(t *testing.T) {
	requireLauncherPublicBindOptIn(t)
...
func requireLauncherPublicBindOptIn(t *testing.T) {
	t.Helper()
	if runtime.GOOS == "windows" && os.Getenv("FACET_TEST_PUBLIC_BIND") != "1" {
		t.Skip("public-interface listener test is opt-in on Windows; set FACET_TEST_PUBLIC_BIND=1")
	}
}
